...
let single_arg_execute =
  let query =
    (let open Caqti_request in exec) ((let open Caqti_type in string)
      [@ocaml.warning "-33"])
      "\n      UPDATE users\n      SET username = ?\n      " in
  let wrapped ~username  ((module Db)  : (module Caqti_lwt.CONNECTION)) =
...
let many_arg_get_one_repeated_arg =
  ...
  let wrapped ~id  ~username  ((module Db)  : (module Caqti_lwt.CONNECTION))
    =
    let f result =
...
let list =
  ...
  wrapped
let collect_list =
  let wrapped ~versions  ((module Db)  : (module Caqti_lwt.CONNECTION)) =
    match versions with
    | [] ->
        Lwt_result.fail
          (let open Caqti_error in
...
let get_cards =
  ...
  let wrapped ~suit  ((module Db)  : (module Caqti_lwt.CONNECTION)) =
    let f result =
...
let get_single_function_out loaders =
  let query =
    (let open Caqti_request in collect) ((let open Caqti_type in unit)
      ...
      [@ocaml.warning "-33"])
      "\n      SELECT id, name\n      FROM users\n      " in
  let wrapped loader () ((module Db)  : (module Caqti_lwt.CONNECTION)) =
    let f result =
