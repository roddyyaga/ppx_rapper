  ...
  let query =
    ...
    exec)
      ((let open Caqti_type in
       tup2 string (tup2 string (tup2 (option string) int))) [@ocaml.warning
                                                               "-33"])
      "\n\
      \      UPDATE users\n\
      ...
      \      "
  in
  let wrapped ~username ~email ~bio ~id (module Db : Caqti_lwt.CONNECTION) =
...
let single_arg_execute =
  let query =
    ...
    exec)
      ...
      "\n      UPDATE users\n      SET username = ?\n      "
  in
  let wrapped ~username (module Db : Caqti_lwt.CONNECTION) =
...
let many_arg_get_one =
  let query =
    ...
    find)
      ...
      ((let open Caqti_type in
       tup2 int (tup2 string (tup2 (option string) bool))) [@ocaml.warning
                                                             "-33"])
      "\n\
      ...
      \      "
  in
  let wrapped ~username ~min_id (module Db : Caqti_lwt.CONNECTION) =
    let f result =
      match result with
...
let many_arg_get_one_repeated_arg =
  let query =
    ...
    find)
      ...
      \      "
  in
  let wrapped ~id ~username (module Db : Caqti_lwt.CONNECTION) =
    let f result =
  ...
  let query =
    ...
    find_opt)
      ((let open Caqti_type in
       tup2 string int) [@ocaml.warning "-33"])
      ((let open Caqti_type in
       tup2 int (tup2 string (tup2 bool (option string)))) [@ocaml.warning
                                                             "-33"])
      "\n\
      ...
      \      "
  in
  let wrapped ~wrong_user ~min_id (module Db : Caqti_lwt.CONNECTION) =
    let f result =
...
let list =
  let wrapped ~following ~ids (module Db : Caqti_lwt.CONNECTION) =
    match ids with
    | [] ->
        Lwt_result.fail
    ...
    | elems ->
        ...
        let f result =
          ...
          match result with Ok x -> Ok (f x) | Error e -> Error e
        in
        Lwt.map f (Db.find_opt query (following, ids))
  in
  wrapped

let collect_list =
  let wrapped ~versions (module Db : Caqti_lwt.CONNECTION) =
    match versions with
    | [] ->
        Lwt_result.fail
          (let open Caqti_error in
...
let get_cards =
  ...
  in
  let wrapped ~suit (module Db : Caqti_lwt.CONNECTION) =
    let f result =
      let g (id, suit) = (id, suit) in
      let f = Stdlib.List.map g in
      match result with Ok x -> Ok (f x) | Error e -> Error e
    in
    Lwt.map f (Db.collect_list query suit)
  in
  wrapped
                              ...
                              (tup2 ptime
                                 (tup2 ptime_span
                                    (tup2 Caqti_type_calendar.cdate
                                       Caqti_type_calendar.ctime))))))))))) [@ocaml.warning
                                                                              "-33"])
  ...
  let query =
    ...
    collect)
      ((let open Caqti_type in
       unit) [@ocaml.warning "-33"])
      ((let open Caqti_type in
       tup2 int (tup2 string (tup2 int (tup2 string int)))) [@ocaml.warning
                                                              "-33"])
      "\n\
...
let get_single_function_out loaders =
  let query =
    ...
    collect)
      ...
      "\n      SELECT id, name\n      FROM users\n      "
  in
  let wrapped loader () (module Db : Caqti_lwt.CONNECTION) =
    let f result =
...
let get_multiple_function_out loaders =
  let query =
    ...
    collect)
      ...
      \      "
  in
  let wrapped (loader, loader') () (module Db : Caqti_lwt.CONNECTION) =
    let f result =
